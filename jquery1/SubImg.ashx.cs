...
{
    /// <summary>
    /// Summary description for SubImg
    /// </summary>
    public class SubImg : IHttpHandler
    {

        public void ProcessRequest(HttpContext context)
        {
            //D:\jquery1\jquery1\SubImg
            const string basePath = @"\\D:\jquery1\jquery1\SubImg\";
            const string baseUrl = @"/jquery1/jquery1/";
            context.Response.ContentType = "text/plain";
            // context.Response.Write("Hello World");
            try
            {
                string dirFullPath = HttpContext.Current.Server.MapPath("~/SubImg/");
                string[] files;
                int numFiles;
                ...
                foreach (string s in context.Request.Files)
                {
                    ...
                    if (!string.IsNullOrEmpty(fileName))
                    {
                        ...
                        else
                        {
                            ...
                            pathToSave_100 = HttpContext.Current.Server.MapPath("~/SubImg/") + mynewfilename;
                            //   Server.MapPath("~/Uploads/User_" + userId + "/");
                            // str_image = "http:/SubImg/" + mynewfilename;
                            str_image = "http://localhost:34128/SubImg/" + mynewfilename;
                            //  str_image = string.Format("{0}/{1}", HttpContext.Current.Server.MapPath("~/SubImg/"), fileName);
